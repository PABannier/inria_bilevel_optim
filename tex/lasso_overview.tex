...
\\
Let's consider a partitioning of the features such that we can partition the columns of $\mathbf{\Phi}$ as $\{\phi_{1}, \dots, \phi_{J}\}$ where $\phi_{j} \in \bbR^{n \times p_j}$ and
...
favorable weighting matrices $\mathbf{W} \in \bbR^{p \times p}$.
\\
...
\jo{to avoid ambiguity you might use $\diag(w)$ for a matrix multiplication in the update formula of the algorithm}
