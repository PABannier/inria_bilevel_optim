...
\begin{equation*}
    \hat{\lambda} = \argmin_{\lambda \in \mathcal{D}} \norm{ \mathbf{Y} - \mathbf{\Phi}\mathbf{X}^{(\lambda)} }_{\text{F}}^2 + 2 \sigma^2 \text{df}(\bf{\mathbf{\Phi}\mathbf{X}^{(\lambda)}})
    \enspace .
\end{equation*}
%
...
\begin{equation*}
    ...
    \left \langle
            \frac{\partial \mathbf{\Phi X}_{:t} }{
            \partial \mathbf{Y}_{:t}} \Delta_t, \Phi \Phi^T \Delta_t
    \right \rangle
    \enspace .
\end{equation*}
%
...
of $\mathbf{X} \in \mathbb{R}^{p\times T}$ which depends on $\mathbf{Y} \in \bbR^{n \times T}$ (\textit{e.g.} a \nameref{section_5} fit for which the divergence
